    ...
    endpoints: {
      "/translate": {
        ...
        examples: {
          ...
          post: {
            url: "/translate?token=your_access_token", // token 通过 query 参数传递
            body: { text: "Hello", source_lang: "en", target_lang: "zh" },
            headers: {
              "Content-Type": "application/json",
              Authorization: "Bearer your_access_token", // 也可以通过 Authorization 头传递 token
            },
          },
        },
      },
...
export async function handleTranslateRequest(
  ...
  try {
    ...
    if (ACCESS_TOKEN) {
      ...
      if (ACCESS_TOKEN !== (token || bearerToken)) {
        if (verbose) {
